<template>
  <view class="enterprise-signature">
    <view></view>
    <info-form :identifyFlag="identifyFlag" tipsText="您即将签署合同，请确认以下信息是否正确：" @toNext="toNext">
        <template v-slot:form>
          <view class="title">企业信息</view>
          <uni-forms ref="form" :modelValue="formData" label-align="right">
              <uni-forms-item label="企业名称" name="enterpriseName">
                  <uni-easyinput type="text" disabled  v-model="formData.enterpriseName" />
              </uni-forms-item>
              <uni-forms-item label="企业信用码" name="socialCreditCode">
                <uni-easyinput type="text" disabled  v-model="formData.socialCreditCode" />
              </uni-forms-item>
          </uni-forms>
          <view class="title">经办人信息</view>
          <uni-forms ref="form" :modelValue="formData" label-align="right">
              <uni-forms-item label="姓名" name="personName">
                  <uni-easyinput type="text" disabled  v-model="formData.personName" />
              </uni-forms-item>
              <uni-forms-item label="身份证号码" name="certificateNum">
                <uni-easyinput type="text" disabled  v-model="formData.certificateNum" />
              </uni-forms-item>
              <uni-forms-item label="手机号码" name="phoneNum">
                  <uni-easyinput type="text" disabled  v-model="formData.phoneNum" />
              </uni-forms-item>
          </uni-forms>
        </template>
      </info-form>
  </view>
</template>

<script>
import InfoForm from '@/components/info-form.vue';
export default {
  name: 'ElectronicSignatureInfoConfirm',
  components: {InfoForm},
  data () {
    return {
      identifyFlag: true,
      formData: {
        enterpriseName: 'AAAA企业',
        socialCreditCode: '91510100BB888GT88M',
        personName: '甘浩添',
        certificateNum: '513029199210020453'.replace(/^(.{6})(?:\d+)(.{0})$/, '$1************$2'),
        phoneNum: '17711445250'.replace(/^(.{3})(?:\d+)(.{3})$/, '$1*****$2')
      }
    }
  },
  methods: {
    toNext () {}
  }
}
</script>

<style lang="scss" scoped>
.enterprise-signature {
  padding: 20rpx;
  .title {
    text-align: center;
    background: #F5F5F5;
    height: 80rpx;
    line-height: 80rpx;
    width: 100vw;
    margin-left: -20rpx;
  }
  /deep/ .uni-forms {
    .uni-easyinput__content {
      text-align: right;
      border: none;
      color: #333;
    }
  }
}
</style>